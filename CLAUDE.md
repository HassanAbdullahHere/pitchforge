...
- Rate limiting (`slowapi`) — `/analyze`: 7/day per user (count of `usage_events WHERE phase='analysis'` — decoupled from proposal deletes, survives restarts) + 14/day per IP; `/auth/google`: 10/hour per IP; `POST /api/profile`: 8/day IP + 4/day per-user; `POST /api/profile/parse-resume`: 4/day IP + 2/day per-user; `app/limiter.py` holds the shared `Limiter` instance
...
- GitHub Actions CI/CD — `.github/workflows/ci-cd.yml`; 6 jobs with path-based skipping: `changes` (always runs first — uses `dorny/paths-filter@v3` to emit `backend` bool for `backend/**`, `pitchforge/**`, `Dockerfile`, `.dockerignore` changes, and `frontend` bool for `frontend/**` changes) → `test-backend` (only when `backend == 'true'` — pgvector/pgvector:pg16 service container + uv cache + pytest) + `test-frontend` (only when `frontend == 'true'` — npm cache + Vitest, runs in parallel with test-backend) → `security-scan` (uses `always()` so skipped test jobs don't block it; passes if both test jobs succeeded or were skipped; skips entirely if neither side changed; Bandit SAST, npm audit, Trivy CVE scan — hard fail on HIGH/CRITICAL only) → `deploy-backend` (only when `backend == 'true'` AND `security-scan` passed AND main push — OIDC auth → ECR push with `previous` tag saved for instant rollback → SSM send-command to EC2: ECR login + pull latest + stop+rm old container + `~/run-backend.sh`) → `deploy-frontend` (only when `frontend == 'true'` AND `deploy-backend` succeeded or was skipped AND main push — health poll every 10s up to 5 min only when backend was also deployed this run, then `vercel deploy --prod` via Vercel CLI); deploy jobs gated to `main` push via `github.ref` + `github.event_name`; `paths-ignore: ['**.md', 'docs/**']` skips entire pipeline on doc-only pushes; Vercel auto-deploy disabled via `exit 0` Ignored Build Step — deploy hook approach abandoned because Ignored Build Step also canceled hook-triggered builds; Vercel CLI bypasses it entirely
