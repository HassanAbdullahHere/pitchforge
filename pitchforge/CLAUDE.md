...
└── chromadb/               # Auto-generated by setup_rag.py — gitignored
                            # ⚠️ Planned migration → pgvector (Step 2)
```
...
⚠️ **Step 2 (planned):** ChromaDB + BM25 corpus → replaced by pgvector + PostgreSQL full-text search.
`retriever.py` and `setup_rag.py` will be rewritten; `chromadb/` directory will be removed.
